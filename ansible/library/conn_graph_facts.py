            ...
            for start_port, link in device_links.items():
                ...
                if l1_start_device != l1_end_device:
                    logging.debug("Found L1 connected port pairs not using the same L1 device: "
                                  "{}:{} <-> {}:{} "
                                  "on L1 devices {} and {}".format(start_device, start_port, end_device, end_port, l1_start_device, l1_end_device))
                    continue

                logging.debug("Found L1 cross connect: {}:{start_port} <->"
                              " {}:{} on L1 device {l1_start_device}".format(start_device, start_port, end_device, end_port))
