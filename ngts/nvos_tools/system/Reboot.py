...
class Reboot(BaseComponent):
    ...
    def action_reboot(self, engine=None, device=None, params="", should_wait_till_system_ready=True, recovery_engine=None, topology_obj=None):
        with allure.step('Execute action for {resource_path}'.format(resource_path=self.get_resource_path())):
            ...
            res_obj = SendCommandTool.execute_command(self.api_obj[TestToolkit.tested_api].action_reboot,
                                                      ...
                                                      params, should_wait_till_system_ready, recovery_engine, topology_obj)
            end_time = time.time()
