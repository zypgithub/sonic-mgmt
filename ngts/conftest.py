...
def topology_obj(setup_name, request, is_performance):
    ...
    with allure.step('Creating topology object'):
        ...
        topology = get_topology_from_noga(request.session, slow_cli=False, override_type=cli_type, force_update=True)
...
def platform_params(show_platform_summary, setup_name, topology_obj):
    ...
    platform_data.filtered_platform = re.search(
        r"(msn\d{4}a\w?|msn\d{4}c|msn\d{4}|sn\d{4}\_[a-z]+|sn\d{4}[a-z]?|qm\d{4}|q\d{4}|mqm\d{4}|mbf.*c|900.*a|bf.*dpu|N5110_LD|N5100_LD|N5112_LD|N5200_LD|N5600_LD|N5500_LD|N6150_LD|[Nn]6100_LD)",
