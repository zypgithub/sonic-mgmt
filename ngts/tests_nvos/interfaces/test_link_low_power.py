...
def test_link_low_power_enabled(engines, devices, prepare_traffic, test_api):
    """
...
@pytest.mark.interface
@pytest.mark.parametrize('test_api', ApiType.ALL_TYPES)
def test_link_low_power_disabled(engines, devices, topology_obj, prepare_traffic, test_api):
    """
    testing disabling the feature and verify show command and ports

    Test flow:
    1. Run nv set fae ib link-low-power state disabled + apply
    2. Run nv show ib link-low-power
    3. validate feature disabled
    4. validate for all ports l1_cap = 0
    5. Run nv unset fae ib link-low-power state + apply
    6. Run nv show ib link-low-power
    7. validate feature enabled
    8. validate for all ports l1_cap = 1
    """
    TestToolkit.tested_api = test_api
    fae = Fae()
    ...
    try:
        with allure.step("disabled l1 saving power capability"):
            fae.ib.link_low_power.set(op_param_name=IbInterfaceConsts.LINK_STATE, op_param_value=NvosConst.DISABLED, apply=True)
