...
from ngts.nvos_tools.Devices.IbDevice import JulietNonScaleoutSwitch, JulietSwitch
...
def show_interface_and_validate(engines, devices, ports_list, command=""):
    output_dictionary = OutputParsingTool.parse_show_all_interfaces_output_to_dictionary(
        Port.show_interface(engines.dut, fae_param=command)
    ).get_returned_value()
...
def toggle_port_state(selected_port, port_state, test_name=""):
    ...
    with allure_step(f"Wait till port {selected_port} is {port_state}"):
        res_obj, duration = OperationTime.save_duration(
            f"port goes {port_state}", "", test_name, selected_port.interface.wait_for_port_state, port_state, sleep_time=0.2
        )
        res_obj.verify_result()
        OperationTime.verify_operation_time(duration, f"port goes {port_state}").verify_result()
...
def validate_ports_state_and_speed(speed, expected_ports: list, prefix: str, state=NvosConsts.LINK_STATE_UP):
    ...
    actual_ports = [
        port.name for port in Port.get_list_of_ports(port_requirements_object=port_requirements) if port.name.startswith(prefix)
    ]

    ValidationTool.validate_subset_in_superset(expected_ports, actual_ports).verify_result()


def validate_mode_set(selected_port, config, mode: str, timeout: int | None = None):
    with allure.step(f"Validate mode {mode} is applied"):
        output = OutputParsingTool.parse_json_str_to_dictionary(selected_port.port.interface.link.phy_recovery.show()).get_returned_value()
...
def select_random_nvl_port_name(devices, prefix=None):
    with allure.step(f"Select {devices.dut.nvl_port_type} port in up state"):
        port_names = [
            port.name
            for port in RandomizationTool.select_random_ports(
                requested_ports_type=devices.dut.nvl_port_type, num_of_ports_to_select=0
            ).get_returned_value()
        ]
        if prefix:
