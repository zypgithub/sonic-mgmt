...
@pytest.mark.interface
@pytest.mark.multiplanar
def test_interface_xdr_slow_speed_trunk_ports(engines, devices, random_api, setup_name, standalone_system: bool, has_loopbox: bool):
    skip_if_no_access_links(has_loopbox, standalone_system)
    ...
    with allure.step(f"Select {devices.dut.nvl5_port_type} ports"):
        port_names = [
            ...
            for port in RandomizationTool.select_random_ports(
                requested_ports_type=devices.dut.nvl5_port_type, num_of_ports_to_select=0
            ).get_returned_value()
    ...
    # Unset port speed and verify default (400G) is restored
        ...
        with allure.step('verify that client received the xdr speed in the existing streaming session'):
            ...
            with allure.independent_step(f'check that "{IbInterfaceConsts.XDR_SLOW_SPEED}" was streamed'):
                verify_msg_in_out_or_err('200', out, err)
