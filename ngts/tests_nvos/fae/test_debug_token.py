...
def test_debug_token_upload_good_flow(engines, test_name, random_api):
    ...
    with allure.step('try to upload debug info {} to {} - Positive Flow'.format(filename, upload_path)):
        debug_image.files.file_name[filename].action_upload(upload_path=upload_path).verify_result()
...
def test_debug_token_upload_bad_flow(engines, test_name, random_api):
    ...
    with allure.step('try to upload debug info to invalid url - url is not in the right format'):
        debug_image.files.file_name['nonexist'].action_upload(
...
def test_debug_info_rename_good_flow(engines, test_name, random_api):
    ...
    with allure.step('try to rename debug info {} to {} - Positive Flow'.format(filename, new_filename)):
        debug_image.files.file_name[filename].action_rename(new_name=new_filename).verify_result()
