...
from ngts.nvos_constants.constants_nvos import ApiType, CpoConsts
...
def test_show_fae_platform_cpo(engines, devices, nv_command, test_api, get_els_list):
    ...
    with allure.step("Test ELS initialization show commands"):
        with allure.independent_step("Test nv show fae system cpo els-initialization"):
            ...
            with allure.step("Pick one random ELS and verify its structure"):
                random_els = random.choice(list(els_init_output.keys()))
                ValidationTool.compare_dictionaries(els_init_output[random_els], CpoConsts.ELS_INIT_DEFAULT_DICT)
        ...
        with allure.independent_step("Test nv show fae system cpo els-initialization-per-laser"):
            ...
            with allure.step("Pick one random ELS and verify its structure"):
                random_els = random.choice(list(els_per_laser_output.keys()))
                els_init_per_laser_data = els_per_laser_output[random_els][CpoConsts.ELS_INITIALIZATION]
                ...
                for laser in els_init_per_laser_data:
                    ValidationTool.compare_dictionaries(els_init_per_laser_data[laser], CpoConsts.ELS_INIT_PER_LASER_DEFAULT_DICT)
...
def test_els_unplug_plug_event(engines, devices, nv_command, test_api, get_els_list):
    ...
    try:
        ...
        with allure.step("Verify link state is up"):
            pass  # TODO: Add verification for link state up
        ...
        with allure.step("Verify link state is down"):
            pass  # TODO: Add verification for link state down
