...
def test_platform_firmware_bmc_component(engines, devices, test_api):
    ...
    with allure.step("Test output of nv show platform firmware and nv show platform firmware"):
    ...
    with allure.step("Grep version per each component and compare version with regular and output"):
        for component in bmc_firmware_inventory:
            ...
            _check_version_in_regular_output(bmc_component_version, firmware_output, component)

    with allure.step("Grep version per BMC component and verify it in regular and command"):
        ...
        firmware_component_output = OutputParsingTool.parse_json_str_to_dictionary(platform.firmware.show('BMC')) \
            .get_returned_value()
        _check_version_in_regular_output(bmc_component_version, firmware_component_output, 'BMC')
...
def _check_version_in_regular_output(bmc_component_version, firmware_output, component=''):
