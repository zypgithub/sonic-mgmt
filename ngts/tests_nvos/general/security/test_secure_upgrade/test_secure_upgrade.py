    ...
    with allure.step(f"install non secure image - expect fail, image path = {img}"):
        ...
        try:
            with allure.step("Attempting installing non secure image"):
                system.image.files.file_name[img_name].action_install(reboot_params=False, expected_output="Failed to verify image signature").verify_result(should_succeed=False)
        finally:
