...
from ngts.nvos_constants.constants_nvos import ApiType, ClusterApps, TestFlowType
...
from ngts.tests_nvos.general.security.nmx_cert.helpers import (
    ...
    update_cluster_app_manager_state,
    verify_cacert_show,
    verify_cert_show,
...
from ngts.tests_nvos.system.gnmi.conftest import get_scp_player


@pytest.mark.nmx
@pytest.mark.security
@pytest.mark.parametrize(
    "test_api, app_name, ca_type",
    [(api, app, random.choice(optional_cacert_types())) for api in ApiType.ALL_TYPES for app in ClusterApps.ALL_APPS],
)
        ...
        with allure.independent_step("Verify in manager show that related fields restored to default"):
            ...
            with allure.independent_step("verify files and fields in json"):
                verify_files(
                    ...
                    engines.dut,
                    {
                        consts.user_config_json_fields.state: consts.fields_that_must_exist_in_user_config_json[
                            consts.user_config_json_fields.state
                        ]
                    },
                )
...
def test_cluster_app_mngr_security_cli_fail_when_cluster_off(test_api, app_name, ca_type):
    ...
    with allure.step("enable cluster and verify all fields were not changed and still default"):
        ...
        with allure.independent_step("verify manager show"):
            verify_manager_show(
                app_name,
                expect_state=consts.fields_that_must_exist_in_user_config_json[consts.user_config_json_fields.state],
...
def test_update_cluster_app_mngr_security_bad_param(test_api, app_name, ca_type):
    ...
    with allure.step("Run update to cert-id that was not imported - expect fail"):
        ...
        with allure.step("Verify in show that related fields don’t change"):
            with allure.independent_step("verify manager show"):
                verify_manager_show(
                    app_name, expect_cert=Defaults.CERT, expect_cacert=Defaults.CACERT, expect_encryption=Defaults.ENCRYPTION
                )
            with allure.independent_step("verify cert show"):
                verify_cert_show(app_name, expect_cert_id=Defaults.CERT)
            with allure.independent_step("verify cacert show"):
...
@pytest.mark.nmx
@pytest.mark.security
@pytest.mark.parametrize("app_name, ca_type", [(app, random.choice(optional_cacert_types())) for app in ClusterApps.ALL_APPS])
def test_cluster_app_mngr_connection(app_name, ca_type):
    ...
    class Test:
        ...
        def run_client_and_verify(self):
            run_manager_hello_request(
                app_name, self.client_mode, self.server_cert, self.server_ca, self.client_cert, self.client_ca
            ).verify_result(self.expect_success)
    ...
    with allure.step("Verify updated values in show kept"):
        for app_name in apps:
            with allure.independent_step(f"app: {app_name}"):
                ...
                with allure.independent_step("verify manager show"):
                    verify_manager_show(
                        app_name, expect_cert=app_cert.name, expect_cacert=app_cert.cacert_name, expect_encryption=encryption_mode[app_name]
                    )
                with allure.independent_step("verify cert show"):
...
def setup_cluster_app_mngr_security_checker(engines):
    ...
    with allure.step("prepare certs"):
        tmp_certs_dir, nmx_certs = setup_certs_for_tests("nmx", ["nmx-cert1", "nmx-cert2"], engines, dut_hostname, False, scp_player)
        certs: dict[str, CertInfo] = {ClusterApps.NMX_CONTROLLER: nmx_certs[0], ClusterApps.NMX_TELEMETRY: nmx_certs[1]}
                        ...
                        with allure.independent_step("Verify values in show restored to defaults"):
                            if app_name == ClusterApps.NMX_TELEMETRY:
                                ...
                                verify_manager_show(
                                    ...
                                    expect_cacert=Defaults.CACERT,
                                    expect_encryption=Defaults.ENCRYPTION,
                                    expect_state=DISABLED,
                                )
                        if app_name == ClusterApps.NMX_TELEMETRY:
                            # NMX Telemetry is ENABLED by default, so client connection with disabled encryption SHOULD work
                            with allure.independent_step("verify client connection with disabled encryption works"):
                                run_manager_hello_request(
                                    app_name,
                ...
                for app_name in ClusterApps.ALL_APPS:
                    with allure.independent_step(app_name):
                        ...
                        with allure.independent_step("Verify values in show kept"):
                            verify_manager_show(
                                app_name, expect_cert=cert.name, expect_cacert=cert.cacert_name, expect_encryption=encryption_mode
                            )
                        with allure.independent_step(f"verify client connection: client mode: {encryption_mode}. expect success: True"):
                            run_manager_hello_request(
                                app_name, encryption_mode, cert, cert, cert, cert, skip_etc_mapping=True
                            ).verify_result()
    finally:
        cleanup_certs_for_tests(tmp_certs_dir, nmx_certs)
...
def test_cluster_app_mngr_connection_combined(ca_type):
    ...
    class Case:
        ...
        def verify(self):
            with allure.step("verify that client of each app can connect only with the matching configuration"):
        ...
        def __verify_goodflow(self):
            with allure.step("good flow: check clients with matching configurations"):
                with allure.independent_step(nmx_c):
                    run_manager_hello_request(nmx_c, self.c_encryption, self.c_cert, self.c_ca, self.c_ca, self.c_cert).verify_result(True)
                with allure.independent_step(nmx_t):
                    run_manager_hello_request(nmx_t, self.t_encryption, self.t_cert, self.t_ca, self.t_ca, self.t_cert).verify_result(True)

        def __verify_badflow(self):
            with allure.step("bad flow: check clients with opposite configurations"):
                with allure.independent_step(nmx_c):
                    run_manager_hello_request(nmx_c, self.t_encryption, self.t_cert, self.t_ca, self.t_ca, self.t_cert).verify_result(False)
                with allure.independent_step(nmx_t):
                    run_manager_hello_request(nmx_t, self.c_encryption, self.c_cert, self.c_ca, self.c_ca, self.c_cert).verify_result(False)
