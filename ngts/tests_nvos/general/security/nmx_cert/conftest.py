...
from ngts.tests_nvos.general.security.certificate.helpers import delete_certificates, import_test_certs
from ngts.tests_nvos.general.security.helpers import add_etc_host_mapping_to_dn, remove_etc_host_mapping_to_dn
from ngts.tests_nvos.general.security.nmx_cert.constants import DISABLED, STATE
...
def skip_unsupported_nmx_device(request, devices):
    """
    Skip NMX tests on devices that don't support NMX.
    Also skip tests for cluster apps not supported on the current device.
    Juliet supports both nmx-controller and nmx-telemetry.
    Rosalind only supports nmx-controller.
    """
    device_type = type(devices.dut).__name__
    has_nmx = getattr(devices.dut, "has_nmx", False)
    if not has_nmx:
        pytest.skip(f"NMX tests are not supported on {device_type} (has_nmx=False)")

    app_name = get_cur_test_param_value(request, "app_name")
    if app_name is None:
        return

    expected_apps = getattr(devices.dut, "expected_cluster_apps", [])
    if app_name not in expected_apps:
        pytest.skip(f"Cluster app '{app_name}' is not supported on {device_type}. Supported apps: {expected_apps}")
