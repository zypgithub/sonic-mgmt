...
from ngts.tests_nvos.general.security.nmx_cert.grpc.config import NMX_T_CONFIG, EncryptionMode, GrpcConfig
from ngts.tests_nvos.general.security.nmx_cert.grpc.nmx_t.server.TelemetryServiceServicer import \
    TelemetryServiceServicer
from ngts.tests_nvos.general.security.nmx_cert.grpc.utils.logs import standalone_logger


class NmxTelemetryServerApp:
    def __init__(self, config, logger=standalone_logger):
        self.name = 'NMX-T SERVER'
        self.config = config
    ...
    def _serve(self):
        ...
        servicer = TelemetryServiceServicer(self.name, self.config, self.logger)
        pb_grpc.add_TelemetryServiceServicer_to_server(servicer, server)
...
def run_grpc_server_app(config: GrpcConfig, logger=None):
    NmxTelemetryServerApp(config, logger).run()


if __name__ == '__main__':
    run_grpc_server_app(NMX_T_CONFIG)
