...
def check_install_and_upgrade_steps_intervals():
    ...
    """
    InstallStepsTimer.analyze_saved_timestamps()

    # Constant dictionary for step limits  # TODO: give more accurate time limits
    intervals_limit: Dict[Tuple[str, str], float] = {
        (InstallSteps.ONIE_NOS_INSTALL, InstallSteps.SYSTEM_IS_READY_AFTER_MANUFACTURE): 20 * MINUTE,
        (InstallSteps.UPGRADE_CMD, InstallSteps.SHUT_DOWN): 20 * MINUTE,
        (InstallSteps.UPGRADE_CMD, InstallSteps.SYSTEM_IS_READY_AFTER_UPGRADE): 20 * MINUTE,
    }
