...
def test_cluster_default_factory_reset(engines, devices, test_api):
    ...
    with allure.step("Create Cluster object"):
        ...
        control_plane_files_deleted = False
    try:
        ...
        if not control_plane_files_deleted:
...
def test_cluster_factory_reset_keep_all_config(engines, devices, test_api, test_name):
    ...
    try:
        with allure.step("Add data before reset factory"):
            username = add_verification_data(engines.dut, system)
        log_level = reset_factory_pre_steps(engines, devices, test_api, cluster, current_time, system, control_plane, all_state_files_paths, all_config_files_paths, output_format, initial_config_contents)
...
def reset_factory_pre_steps(engines, devices, test_api, cluster, current_time, system, control_plane, all_state_files_paths, all_config_files_paths, output_format, initial_config_contents):
...
def verify_config_files_content_not_changed(control_plane, initial_config_contents, engines):
...
def delete_all_control_plane_fetched_generated_files(control_plane, all_config_files_paths, all_state_files_paths):
