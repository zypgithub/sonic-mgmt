...
def test_syslog_rate_limit_stress_test(test_api):
    """
    Test Objective:
    Verify rate limiting behavior with include/exclude filters across multiple servers under stress conditions.

    Test flow:
    1. Create server with TCP protocol and port 514
    2. Create 5 selectors with different configurations and rate limits
    3. Configure each selector with their respective filters
    4. Attach all selectors to the same server with different priorities
    5. Send 1000 critical messages
    6. Verify memory usage and log file size
    7. Clean up configurations
    """
    TestToolkit.tested_api = test_api
    system = System()
    ...
    num_selectors = 5
    num_messages = 100
    wait_time = 120  # 1 minute in seconds

    # Define facilities and their corresponding filters
    facilities = {
        'user': {'filter': 'test.', 'severity': SyslogSeverityLevels.INFO},
        'local0': {'filter': 'error.', 'severity': SyslogSeverityLevels.ERROR},
        'kern': {'filter': 'kernel.', 'severity': SyslogSeverityLevels.CRITICAL},
        'mail': {'filter': 'mail.', 'severity': SyslogSeverityLevels.WARN},
        'daemon': {'filter': 'debug', 'severity': SyslogSeverityLevels.DEBUG}
    }

    # Define rate limit configurations
    rate_limits = [
        {'interval': 65535, 'burst': 20},    # High interval, low burst
        {'interval': 20, 'burst': 65535},    # Low interval, high burst
        {'interval': 100, 'burst': 100},     # Balanced
        {'interval': 10, 'burst': 1},        # Very restrictive
        {'interval': 65535, 'burst': 65535}          # Less restrictive
    ]

    # Generate selector configurations dynamically
    selector_configs = []
    for i in range(num_selectors):
        facility = list(facilities.keys())[i]
        selector_configs.append({
            'id': f'selector-{i + 1}',
            'priority': i + 1,
            'facility': facility,
            'severity': facilities[facility]['severity'],
            'filter': facilities[facility]['filter'],
            'rate_limit': rate_limits[i]
        })

    try:
        with allure.step("Create server with TCP protocol and port 514"):
            system.syslog.servers.set_server(server_id, apply=True)
            system.syslog.servers.servers_dict[server_id].set_protocol(SyslogConsts.TCP, apply=True)
            system.syslog.servers.servers_dict[server_id].set_port(SyslogConsts.DEFAULT_PORT, apply=True)

        with allure.step("Take initial memory statistics"):
            initial_memory = remote_server_engine.run_cmd('free -m')
            initial_log_size = remote_server_engine.run_cmd('du -k /var/log/syslog')

            # Parse initial memory values
            initial_mem_lines = initial_memory.split('\n')
            for line in initial_mem_lines:
                if 'Mem:' in line:
                    initial_mem_values = line.split()
                    initial_used_mem = int(initial_mem_values[2])  # Used memory in MB
                    break

        with allure.step("Create and configure selectors"):
            for config in selector_configs:
                ...
                create_selector_configuration(
                    ...
                    filters=[('include', config['filter'])],
                    rate_limit_interval=config['rate_limit']['interval'],
                    rate_limit_burst=config['rate_limit']['burst']
                )

                # Attach selector to server
                system.syslog.servers.servers_dict[server_id].set_selector_priority(
                    config['priority'], config['id'], apply=True)

        with allure.step(f"Send {num_messages} critical messages"):
            for i in range(num_messages):
                test_message = f"debug_test_message_{i}"
                send_msg_to_server(test_message, server_id, remote_server_engine,
                                   priority=SyslogSeverityLevels.CRITICAL, verify_msg_received=True)

        with allure.step(f"Sleep for {wait_time} seconds"):
            time.sleep(wait_time)

        with allure.step("Check memory usage and log file size"):
            final_memory = remote_server_engine.run_cmd('free -m')
            final_log_size = remote_server_engine.run_cmd('du -k /var/log/syslog')

            # Parse final memory values
            final_mem_lines = final_memory.split('\n')
            for line in final_mem_lines:
                if 'Mem:' in line:
                    final_mem_values = line.split()
                    final_used_mem = int(final_mem_values[2])  # Used memory in MB
                    break
            ...
            assert "no_core" in core_check or not core_check.strip(), \
                "Core dump found after stress test"

    finally:
        with allure.step("Clean up configurations"):
            # Unset all selector priorities
            for config in selector_configs:
                system.syslog.servers.servers_dict[server_id].unset_selector_priority(
                    config['priority'], apply=True)
            # Unset all selectors
            for config in selector_configs:
                system.syslog.selectors.unset_selector(config['id'], apply=True)
            # Unset server
            system.syslog.servers.unset_server(server_id, apply=True)
