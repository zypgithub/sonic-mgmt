...
class SonicPerformanceCli:
    ...
    def __init_(self, topology_obj, engine, dut_alias, cli_obj):
    ...
    def apply_configuration_file(self, src_file, dst_dut_dir="/tmp"):
        logging.info("Applying the configuration_file onto the dut after copying")
        self.engine.copy_file(source_file=src_file, file_system=dst_dut_dir, dest_file="config.json",
                              overwrite_file=True, verify_file=False)
        with allure.step("Apply Sonic configuration"):
            full_path = os.path.join(dst_dut_dir, "config.json")
