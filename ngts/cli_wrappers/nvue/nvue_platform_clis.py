...
class NvuePlatformCli(NvueBaseCli):
    ...
    def action_install_fae_bios_firmware(engine, bios_image_path, resource_path='', device=None, topology_obj=None):
        ...
        """
        return NvuePlatformCli.action_install(engine=engine, device=device, fae_command=True, args='firmware bios files {}'.format(bios_image_path), expect_reboot=True, force=True, topology_obj=topology_obj)
