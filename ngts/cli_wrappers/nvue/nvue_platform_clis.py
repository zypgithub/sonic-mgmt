...
class NvuePlatformCli(NvueBaseCli):
    ...
    def action_install_bios_firmware(engine, bios_image_path, resource_path='', device=None):
        ...
        """
        return NvuePlatformCli.action_install(engine=engine, device=device, fae_command=False, args='firmware BIOS files {}'.format(bios_image_path), expect_reboot=True, force=True)
