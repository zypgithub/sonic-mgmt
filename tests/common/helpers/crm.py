...
THR_VERIFY_CMDS = OrderedDict([
    ("exceeded_used", "bash -c \"crm config thresholds {{crm_cli_res}}  type used; \
         crm config thresholds {{crm_cli_res}} low {{crm_used|int - 1 - dram_margin|int * 2}}; \
         crm config thresholds {{crm_cli_res}} high {{crm_used|int - dram_margin|int}}\""),
    ("clear_used", "bash -c \"crm config thresholds {{crm_cli_res}} type used && \
         crm config thresholds {{crm_cli_res}} low {{crm_used|int + dram_margin|int}} && \
         crm config thresholds {{crm_cli_res}} high {{crm_used|int + 1 + dram_margin|int * 2}}\""),
    ("exceeded_free", "bash -c \"crm config thresholds {{crm_cli_res}} type free && \
         crm config thresholds {{crm_cli_res}} low {{crm_avail|int - 1 - dram_margin|int * 2}} && \
         crm config thresholds {{crm_cli_res}} high {{crm_avail|int - dram_margin|int}}\""),
    ("clear_free", "bash -c \"crm config thresholds {{crm_cli_res}} type free && \
         crm config thresholds {{crm_cli_res}} low {{crm_avail|int + dram_margin|int}} && \
         crm config thresholds {{crm_cli_res}} high {{crm_avail|int + 1 + dram_margin|int * 2}}\""),
