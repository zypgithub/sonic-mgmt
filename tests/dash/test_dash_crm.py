...
def get_crm_facts(dpuhost):
    ...
    def _show_and_parse_crm_resources():
        ...
        if 4 in list(sections.keys()):
            crm_facts['dash_acl_group'] = []
            for line in sections[4]:
                match = re.search(r".*\s+.*\s+\d+\s+\d+", line)
                if match:
                    dash_acl_group = re.split(r'\s+', match.group(0))
                    crm_facts['dash_acl_group'].append(
                        {
                            'dash_acl_group_id': dash_acl_group[0],
                            'resource_name': dash_acl_group[1],
                            'used count': dash_acl_group[2],
                            'available count': dash_acl_group[3]
                        })
        return True
...
class TestDashCRM:
    ...
    def test_dash_acl_rules(self, ip_ver):
        ...
        assert len(dash_acl_group) == self.acl_groups_num, \
            f"Unexpected number of ACL groups. Expected: {self.acl_groups_num}, found:  {len(dash_acl_group)}"
