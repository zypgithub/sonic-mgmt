...
def check_byo_status(status, dut_run_command):
    ...
    output = dut_run_command("docker ps")['stdout']
...
def setup(duthost, tbinfo, dpu_info, platform, enable_dpu_mgmt_forwarding):
    ...
    if dpu_info:
        ...
        with allure.step("Config vlan for the smartswitch dataplane"):
            ...
            dpu_data_port = dpu_info["data_port"]
        ...
        with allure.step("Config dns nameserver for DPU"):
            for nameserver in RESOLV_CONF_NAMESERVERS['public']:
                duthost.dpu_shell(f"sudo config dns nameserver add {nameserver}", module_ignore_errors=True)
    ...
    if dpu_info:
        ...
        with allure.step("Disable byo on the dpu"):
            duthost.shell(f"sudo sonic-byo.py disable")
            pytest_assert(check_byo_status('disabled', duthost.shell), "Failed to disable BYO on DPU.")


@pytest.fixture(scope="function", autouse=True)
def disable_byo(duthost, platform):

    yield

    if platform in SMARTSWITCH_PLATFORMS:
        dut_run_command = duthost.dpu_shell
    else:
        dut_run_command = duthost.shell

    with allure.step("Disable BYO"):
        dut_run_command(f"sudo sonic-byo.py disable")
        pytest_assert(check_byo_status('disabled', dut_run_command), "Failed to disable BYO on DPU.")


def get_docker_image_id(docker, dut_run_command):
    try:
        image_id = re.search(r' \w{12} ', dut_run_command(f"docker image ls {docker}")['stdout']).group().strip()
...
def test_byo(ptfadapter, duthost, option, platform):
    """
    Initiate the byo flow on a standalone dpu and verify the demo provided by the DPDK app.
    ...
    if option != "pull":
        with allure.step(f"Pull the dpdk-app image: {DPDK_APP}"):
            dut_run_command(f"docker pull {DPDK_APP}", module_async=True)
            pull_success = wait_until(docker_pull_timeout, 30, 0, get_docker_image_id, DPDK_APP, dut_run_command)
            pytest_assert(pull_success, f"Failed to pull the docker image: {DPDK_APP}")
            image_id = get_docker_image_id(DPDK_APP, dut_run_command)
    ...
    if option == "pull":
        with allure.step('Enable BYO on the DPU with the "pull" option'):
            ...
            dut_run_command(f"sudo sonic-byo.py enable --pull {DPDK_APP}", module_async=True)
            byo_check_timeout = docker_pull_timeout
    elif option == "file":
        with allure.step('Save the dpdk-app image to a file since the test option is "file"'):
            dut_run_command(f"sudo docker save {image_id} -o /tmp/dpdk-app.gz")
        with allure.step('Remove the image'):
            dut_run_command(f"sudo docker image rm -f {image_id}")
        with allure.step('Enable BYO on the DPU with the "file" option'):
            dut_run_command(f"sudo sonic-byo.py enable --file /tmp/dpdk-app.gz")
    elif option == "image":
        with allure.step('Enable BYO on the DPU with the "image" option'):
            dut_run_command(f"sudo sonic-byo.py enable --image {image_id}")
    ...
    with allure.step("Send a packet with DPU data port mac address as the dst mac address,"
        ...
        data_port_mac_address = dut_run_command("cat /sys/class/net/Ethernet0/address")['stdout']
