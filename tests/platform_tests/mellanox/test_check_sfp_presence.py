...
def test_check_sfp_presence(duthosts, rand_one_dut_hostname, conn_graph_facts, dpu_npu_port_list):     # noqa: F811
    ...
    assert check_presence_output["rc"] == 0, f"Command '{check_intf_presence_command}' failed with return code {check_presence_output['rc']}"
    ...
    assert len(intf_not_present) == 0, f"Missing interfaces in presence output: {intf_not_present}"
    logging.info(f"All expected interfaces are present in the output")
