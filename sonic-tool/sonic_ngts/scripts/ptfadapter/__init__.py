"""This file will overwrite tests/common/plugins/ptfadapter/__init__.py when running dash test on canonical setup"""
import os
import pytest
import time
...
def get_ifaces(netdev_output):
    ...
    for line in netdev_output.split('\n'):
        ...
        if 'eth0' in iface or 'lo' in iface:
            continue
...
def ptfadapter(ptfhosts, tbinfo, request, duthost):
    ...
    for seq, ptfhost in enumerate(ptfhosts):
        ...
        ifaces_map = get_host_a_ptf_port_table(duthost, ifaces)
...
def get_host_a_ptf_port_table(duthost, ptf_ifaces):
    lldp_table_list = duthost.show_and_parse('show lldp table')
    iface_map = {}
    for port in ptf_ifaces:
        for lldp_info in lldp_table_list:
            if port in lldp_info["remoteportdescr"]:
                if lldp_info["localport"] == "Ethernet0":
                    iface_map[0] = port
                else:
                    iface_map[1] = port
    return iface_map
